...
void schedule()
{
    ...
    while (select_Curr != NULL) {
        select_Curr->priority = PRI_USER_MAX - fixedpt_toint(fixedpt_div(select_Curr->estcpu, fixedpt_fromint(4))) - select_Curr->nice * 2;

        if (select_Curr->priority < PRI_USER_MIN)select_Curr->priority = PRI_USER_MIN;
        if (select_Curr->priority > PRI_USER_MAX)select_Curr->priority = PRI_USER_MAX;

        select_Curr = select_Curr->next;
    }
    select_Curr = g_task_head;
    while (select_Curr != NULL) {
        if ((select_Curr->tid != 0) && (select_Curr->state == TASK_STATE_READY))
        {
            if (Curr->priority > select_Curr->priority || Curr == 0) {
                Curr = select_Curr;
            }
        }
        select_Curr = select_Curr->next;
